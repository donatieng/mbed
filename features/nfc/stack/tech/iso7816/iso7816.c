...
#if 0
//DESFIRE kludges
const uint8_t desfire_reply_hw[] = {0xaf, 0x04, 0x01, 0x01, 0x01, 0x00, 0x16, 0x05};
const uint8_t desfire_reply_sw[] = {0xaf, 0x04, 0x01, 0x01, 0x01, 0x03, 0x16, 0x05};
const uint8_t desfire_reply_id[] = {0x00, 0x04, 0x58, 0x6B, 0x21, 0x80, 0x1D, 0x80, 0xCF, 0xA6, 0x97, 0x49, 0x00, 0x37, 0x08};

static bool desfire_kludge(nfc_tech_iso7816_t* pIso7816)
{
  static int step = 0;
  //Kludge DESFIRE
  if( buffer_length(&pIso7816->rx) == 1 )
  {
    buffer_reset(&pIso7816->tx);

    uint8_t b = buffer_read_nu8(&pIso7816->rx);

    if(b == 0x60)
    {
      step = 0;
      buffer_byref(&pIso7816->rApdu.dataOut, &desfire_reply_hw, sizeof(desfire_reply_hw));
    }
    else if(step == 1)
    {
      buffer_byref(&pIso7816->rApdu.dataOut, &desfire_reply_sw, sizeof(desfire_reply_sw));
    }
    else if(step == 2)
    {
      buffer_byref(&pIso7816->rApdu.dataOut, &desfire_reply_id, sizeof(desfire_reply_id));
    }
    step++;
    iso7816_transmit(pIso7816);
    return true;
  }
  return false;
}
...
void iso_dep_stream_transmit_cb(buffer_t* pDataIn, bool* pClose, size_t maxLength, void* pUserParam)
{
  ...
  //Rejoin txSplit and r-apdu buf if needed
  //buffer_join(&pIso7816->rApdu.dataOut, &pIso7816->txSplit);

  //txSplit will be initialized
  //buffer_split(&pIso7816->rApdu.dataOut, &pIso7816->txSplit, maxLength);

  //Return tx buffer
  //*ppDataIn = &pIso7816->rApdu.dataOut;

  //buffer_dump(&pIso7816->rApdu.dataOut);
