...
//
//    //PN512 can report collision here, however we don't want to handle it till end of receive phase
//    uint8_t err_reg = pn512_register_read(pPN512, PN512_REG_ERROR);
//    if( err_reg == 0x08 ) // Collision (and no other error)
//    {
//      irqs &= ~PN512_IRQ_ERR;
//      irqs |= PN512_IRQ_TX;
//    }
...
void pn512_transceive_hw(pn512_t* pPN512, pn512_transceive_mode_t mode, pn512_cb_t cb)
{
  ...
  else
  {
    ...
    irqs_en = PN512_IRQ_TX | /*PN512_IRQ_ERR|*/ PN512_IRQ_IDLE;
    if( PN512_FRAMING_IS_TARGET(pPN512->framing) )
    {
      irqs_en |= PN512_IRQ_RF_OFF;
    }
    ...
    /*if( (mode == pn512_transceive_mode_transmit) || (pn512_cmd_get(pPN512) != PN512_CMD_TRANSCEIVE) )
    {
      pn512_cmd_exec(pPN512, PN512_CMD_IDLE);
      pn512_cmd_exec(pPN512, PN512_CMD_TRANSMIT);
    }*/
