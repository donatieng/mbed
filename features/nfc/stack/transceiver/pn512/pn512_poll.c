...
void pn512_target_anticollision_complete(pn512_t* pPN512, nfc_err_t ret)
{
  ...
  // vTaskPrioritySet(NULL, CFG_TASK_PRIO_NFC);
  ...
  if (iso14443a)
  {
    ...
    if ( pPN512->config.targets.nfc_nfc_dep_a && (b0 == 0xF0) )
    {
      ...
      //Consume byte
      //buffer_n_discard(&pPN512->readBuf, 1);

      //Set specific registers for NFC-DEP
      //pn512_framing_handle_sync_bytes(pPN512); -- NOPE
...
void pn512_target_anticollision(pn512_t* pPN512, pn512_cb_t cb)
{
  ...
  // vTaskPrioritySet(NULL, CFG_TASK_PRIO_NFC_HIGH);
  ...
  //bool autocoll = true;
  //bool iso14443a_initialized = false;
...
//  pn512_register_write(pPN512, PN512_REG_COLL, 0x00); // Set MSB to 0, to accept collisions
...
//  pn512_register_write(pPN512, PN512_REG_COLL, 0x80); // Set MSB to 1, to treat collisions as errors
...
//#define WAIT_FOR_EXTERNAL_FIELD_DURATION 1000 // Wait for 1000 ms for an initiator
